...
# or: curl -fsSL https://cdn.jsdelivr.net/gh/cambridgetcg/whitehack@main/install.sh | bash

set -e
DIR="${1:-$HOME/.whitehack}"
echo "🤍 installing whitehack to $DIR"
mkdir -p "$DIR"
cp -r bin src package.json README.md LEARN.md LOOP.md CONTRIBUTING.md SHARE.md "$DIR/" 2>/dev/null || true
chmod +x "$DIR/bin/whitehack.js" 2>/dev/null || true

# Add to PATH (optional — skip with: NO_PATH=1 bash install.sh)
if [ -z "$NO_PATH" ]; then
  if ! echo "$PATH" | grep -q "$DIR/bin"; then
    SHELL_RC="$HOME/.bashrc"
    [ -f "$HOME/.zshrc" ] && SHELL_RC="$HOME/.zshrc"
    echo "export PATH=\"\$PATH:$DIR/bin\"" >> "$SHELL_RC"
    echo "  added to PATH in $SHELL_RC (skip with NO_PATH=1)"
  fi
fi
...
echo "  invite: https://love-is.axiepro.workers.dev"
echo ""
